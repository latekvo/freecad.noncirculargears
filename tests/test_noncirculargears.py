...
def test_helical_limits(document):
    """Straight teeth cost nothing, teeth that lean too far are refused, and
    a gear with no teeth on it has nothing to lean."""
    ...
    driver.tooth_style = "simple"
    document.recompute()
    check(
        "a simple gear has no teeth to lean and is raised on one section anyway",
        "Invalid" not in driver.State and driver.helix_sections == 1,
        "%s, %d sections" % (driver.State, driver.helix_sections),
    )
    document.removeObject(mate.Name)
    document.removeObject(driver.Name)
